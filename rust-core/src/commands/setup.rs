...
pub async fn setup(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let existing: Option<i64> =
        sqlx::query_scalar("SELECT guild_id FROM guild_config WHERE guild_id = $1")
            .bind(guild_id.get() as i64)
            .fetch_optional(ctx.data().db.as_ref())
            .await?;

    if existing.is_some() {
        ctx.say("⚙️ This server is already configured! Use `/config` to edit settings.")
            .await?;
        return Ok(());
    }

    let guild: serenity::PartialGuild = guild_id.to_partial_guild(ctx).await?;
    ...
    let embed = serenity::CreateEmbed::new()
        ...
        .description("Bot successfully configured. The following channels were created:")
        .field("📋 Log Channel", format!("<#{}>", log_channel.id), false)
        .field(
            "✅ Verification Channel",
            format!("<#{}>", verify_channel.id),
            false,
        )
        .field(
            "🎭 Roles Channel",
            format!("<#{}>", roles_channel.id),
            false,
        )
        .field(
            "Active Features",
