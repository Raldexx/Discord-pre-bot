...
pub async fn rolepanel(ctx: Context<'_>,
    ...
    let embed = serenity::CreateEmbed::new()
        ...
        .description(format!(
            "{}\n\n**Available Roles:**\n{}",
            description.as_deref().unwrap_or("Click the buttons below to add or remove a role."),
            role_list
        ))
        .color(serenity::Colour::BLURPLE)
        .timestamp(serenity::Timestamp::now());

    let buttons: Vec<serenity::CreateButton> = roles.iter()
        .map(|r| serenity::CreateButton::new(format!("role_{}", r.id))
            .label(&r.name)
            .style(serenity::ButtonStyle::Secondary))
        .collect();

    let components: Vec<serenity::CreateActionRow> = buttons.chunks(5)
        .map(|c| serenity::CreateActionRow::Buttons(c.to_vec()))
        .collect();

    ctx.channel_id().send_message(ctx,
        serenity::CreateMessage::new().embed(embed).components(components),
    ).await?;
